var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
import { Component, Input } from "@angular/core";
import { BaseReportComponent } from 'src/app/Component/BaseReportComponent';
import { HubConnect } from "src/app/API/HUBConnect";
let FindCSMain = class FindCSMain extends BaseReportComponent {
    constructor() {
        super();
        this.hubConnect = new HubConnect();
    }
};
__decorate([
    Input()
], FindCSMain.prototype, "AdminMode", void 0);
FindCSMain = __decorate([
    Component({ selector: "Find-CS-Main", templateUrl: "FindCSMain.html" })
], FindCSMain);
export { FindCSMain };
//# sourceMappingURL=FindCSMain.js.map