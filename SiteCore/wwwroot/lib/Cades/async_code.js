        ...
        function(resolve, reject){
            ...
            cadesplugin.async_spawn(function *(arg) {
              ...
              CAPICOM_STORE_OPEN_MAXIMUM_ALLOWED);                   
                    ...
                    alert("Ошибка при поиске сертификатов:"+ex.message);
                    return;
                }
                ...
                for (var i = 1; i <= certCnt; i++) {
                    ...
                    try {
                        ...
                        var isPK =  yield cert.HasPrivateKey();
                        ...
                        if ( FindAlgorithm(yield Algorithm.Value)) {
                            var certObj = new CertificateObj(yield cert.SubjectName,
                                yield cert.IssuerName,
                                ProviderName,
                                yield Algorithm.FriendlyName,
...
function Sign_Async(certSubjectName, SN, HASHTABLE) {
    return new Promise(
        function(resolve, reject) {
            cadesplugin.async_spawn(function*(arg) {
                    ...
                    try {
                        ...
                        var oCertificates =
                            yield CertificatesObj.Find(CAPICOM_CERTIFICATE_FIND_SUBJECT_NAME, certSubjectName);
                        var oCertificate;

                        var Count = yield oCertificates.Count;
                        if (Count > 1) {
                            for (var i = 1; i <= Count; i++) {
                                var s = yield oCertificates.Item(i);
                                var ssn = yield s.SerialNumber;
                                if (ssn === SN)
                        ...
                        var PublicKey = yield oCertificate.PublicKey();
                        var Algorithm = yield PublicKey.Algorithm;


                        var algSing =  FindAlgorithm(yield Algorithm.Value);
                        if (algSing == undefined) {
                            throw 'Алгоритм подписи не поддерживается';
                        }
                        ...
                        yield oHashedData.propset_Algorithm(algSing);
                        yield oHashedData.propset_DataEncoding = cadesplugin.CADESCOM_BASE64_TO_BINARY;
                        //  oHashedData.propset_Algorithm = yield algorithm;
                        //oHashedData.Algorithm = yield algorithm;
                        var SIGNARRAY = {};
                        for (key in HASHTABLE) {
                            var value = HASHTABLE[key];
                            yield oHashedData.SetHashValue(value);
                            try {

                                Signature = yield oSignedData.SignHash(oHashedData, oSigner, CADESCOM_CADES_BES);
                                SIGNARRAY[key] = Signature;
                            } catch (err) {
...
function CheckForPlugIn_Async() {
    ...
    cadesplugin.async_spawn(function *() {
        ...
        //   printSert();  var txtDataToSign = "Hello World";
