...
namespace SiteCore.Class
{
    ...
    public sealed class RequiredIfAttribute : ValidationAttribute
    {
        ...
        {
            return string.Format(CultureInfo.CurrentCulture, base.ErrorMessageString, name, OtherPropertyDisplayName ?? OtherProperty, OtherPropertyValue, IsInverted ? "other than " : "of ");
        }
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            if (!IsInverted && OtherPropertyValue.Count(x => Equals(otherValue, x)) != 0 || IsInverted && !OtherPropertyValue.Any(x => Equals(otherValue, x)))
            {
                switch (value)
                {
                    ...
                    case string val when val.Trim().Length == 0:
                        return new ValidationResult(this.FormatErrorMessage(validationContext.DisplayName));
                }
            }
